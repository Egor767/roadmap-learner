...
class CardService:
    ...
    def __init__(self, repo: "CardRepository", verify: "VerifyRepository", cache: "CacheHelper"):
        self.repo = repo
        self.verify = verify
        self.cache = cache
    ...
    async def get_by_id(self, current_user: "User", card_id: BaseIdType) -> CardRead:
        ...
        card, status = await asyncio.gather(
            ...
            self.repo.get_status(current_user.id, card_id),
        )
    ...
    async def update(self, current_user: "User", card_id: BaseIdType, update_data: CardUpdate) -> CardRead:
        ...
        if status is not None:
            tasks.append(self.repo.update_progress(current_user.id, card_id, status))
